...
## 2026-09-25 — Automated live demo video; RawTree stale-read fix (Claude Code)

- **Product fix found while recording:** On a fresh run, the first `step` after `start` sometimes read an older checkpoint (RawTree read-after-write lag) and failed closed with "RawTree returned stale state". `RawTreeMemory.load` now re-reads up to 5 times with exponential backoff before failing (reads only; writes are still never retried). New test in `tests/rawtree.test.ts`; `npm test` passed 68/68.
- **Live demo video (user chose automated, voice "Zoe (Premium)"):** `exports/demo/continuum-live-demo.mp4`, 2:35. It combines a captured real terminal restart session (fresh run `acme-demo-9989`, fictional pricing, real runtimes) replayed in a terminal-style player, headless-Chrome screencast frames of the real inspector with a drawn cursor and scene captions (real clicks; the Gemma verified answer and the failure question withheld, both answered live by local Liquid), and the brief. Narration by Zoe (Premium) with pauses, written for listening. Tools in `tools/demo-live/`.
- **Issues found and fixed during production:** (1) The inspector server was still running code from before the ask-verification fixes, so the "what failed?" answer showed as verified in the first recording. The server was restarted and everything re-recorded; the server returned `rejected` with the attribution reason before re-recording. (2) Terminal cues were tightened so the replay stays on screen. (3) The `ask` answer was scrolled below the sticky header so the withheld verdict is visible. (4) Assembler bugs: an audio track taken from a released asset (-12780), then out-of-range probe times.
- **Renamed:** the earlier page-traversal video is now `exports/demo/continuum-pitch.mp4` (docs updated).
- **Credit impact:** None (local Liquid, RawTree only).
